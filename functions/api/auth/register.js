...
export async function onRequestPost(context) {
  ...
  if (!tsSetting || tsSetting.value === 'true') {
  ...
  // 邀请码
  const inviteSetting = await db.prepare("SELECT value FROM system_settings WHERE key = 'invite_required'").first();
  if (!inviteSetting || inviteSetting.value === 'true') {
  ...
  try {
    ...
    if (inviteSetting && inviteSetting.value === 'true' && inviteCode) {
