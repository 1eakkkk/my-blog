...
export async function onRequestGet(context) {
  ...
  if (id) {
    // 连表查询，获取作者当前的昵称、头像种子(用户名)、VIP状态、等级
    const post = await db.prepare(`
      SELECT posts.*, users.username as author_username, users.nickname as author_nickname, users.is_vip as author_vip, users.level as author_level
      FROM posts 
    ...
    const posts = await db.prepare(`
      SELECT posts.*, users.nickname as author_nickname, users.is_vip as author_vip 
      FROM posts 
      JOIN users ON posts.user_id = users.id 
