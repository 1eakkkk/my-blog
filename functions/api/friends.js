...
export async function onRequest(context) {
  ...
  if (request.method === 'POST') {
      ...
      if (action === 'add') {
          ...
          await db.prepare('INSERT OR IGNORE INTO friends (user_id, friend_id, status, created_at) VALUES (?, ?, 0, ?)').bind(me.id, target_id, Date.now()).run();
          ...
          return new Response(JSON.stringify({ success: true, message: '已发送申请' }));
      }
